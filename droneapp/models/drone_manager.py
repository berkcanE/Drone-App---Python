...
FRAME_X = int(960/3)
FRAME_Y = int(720/3)
FRAME_AREA = FRAME_Y * FRAME_Y
FRAME_SIZE = FRAME_AREA * 3
FRAME_CENTER_X = FRAME_X / 2
FRAME_CENTER_Y = FRAME_Y / 2

CMD_FFMPEG = (f'ffmpeg - hwaccel auto -hwaccel_device opencl -i pipe:0 -pic_fmt bgr24 -s {FRAME_X}x{FRAME_Y} ' 
             f'-f rawvideo pipe:1')
...
class DroneManager(metaclass=Singleton):
    ...
    def flip_front(self):
        return  self.send_command('flip front')

    def flip_back(self):
          return self.send_command('flip back')

    def flip_left(self):
          return self.send_command('flip left')

    def flip_right(self):
        return  self.send_command('flip right')
